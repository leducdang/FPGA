...
reg   [9:0] addr_uart_img;
reg	[15:0] data_w_uart_img;
reg	wren_uart_img;

wire  [15:0] data_r_uart_img;
...
//	begin
//		if(!reset_n)	// baudrate 115200 bit/s				// nêu sau 1ms không có dữ liệu nhận được thì reset
//		begin
//				counter 	<= 0;
//				addr_uart_img <= 10'd0;
//				stt_rx <= 0;
...
//		else
//			begin
//				counter <= counter + 16'd1;				// tăng biến đếm counter
//				case(stt_rx)
//					4'd0:	if(fb_rx)
//						begin
//							data_w_uart_img <= data_rx;
//							stt_rx <= 1;
//						end
//					4'd1:	if(!fb_rx)							// cho khi uart_read chuẩn bị đọc dữ liệu tiếp theo.
//						begin
//							addr_uart_img <= addr_uart_img + 10'd1;
//							stt_rx <= 0;
//							counter 	<= 0;
//						end
//				endcase
...
//			wren_uart_img <= 1'b0;
			...
			if (fb_rx)
				begin
					data_w_uart_img <= {8'd0, data_rx};
//					wren_uart_img   <= 1'b1;

					if (addr_uart_img < 10'd783)
						begin
							addr_uart_img <= addr_uart_img + 1'b1;
//							done_uart_img <= 1'b1;
						end
...
ram_uart u_ram_uart(
.address_a(addr_uart_img),
.clock_a(clk),
.data_a(data_w_uart_img),
//.wren_a(wren_uart_img),
.wren_a(1'b1),
.q_a(data_r_uart_img),
...
//image uart_image (
//   .address(addr_uart_img),
//   .clock(clk_100),
//   .data(data_w_uart_img),
//   .wren(wren_uart_img),
//   .q(data_r_uart_img)
//);
