...
always@(posedge clock or negedge reset)
	begin
		if(!reset) begin tx<= 1; stt_tx <= 0; end
		else if(run)																		//	module top_level gui data va tín hieu bắt đầu truyền dữ liệu 
			case(stt_tx)
				8'd0: begin tx <= 1; 		stt_tx<=1; end					//	idle
				8'd1: begin tx <= 0; 		stt_tx<=2; end					// start
				8'd2:	begin tx <= data[0]; stt_tx<=3; end					// gui bit lsb - bit[0]
				8'd3:	begin tx <= data[1]; stt_tx<=4; end					//  bit[1]
				8'd4:	begin tx <= data[2]; stt_tx<=5; end					//  bit[2]
				8'd5:	begin tx <= data[3]; stt_tx<=6; end					//  bit[3]
				8'd6:	begin tx <= data[4]; stt_tx<=7; end					//  bit[4]
				8'd7:	begin tx <= data[5]; stt_tx<=8; end					//  bit[5]
				8'd8:	begin tx <= data[6]; stt_tx<=9; end					//  bit[6]
				8'd9:	begin tx <= data[7]; stt_tx<=10; end					//  gui bit msb - bit[7]
				8'd10:	begin tx <= 1; 	stt_tx<=11; end				//  stop
				8'd11:begin tx <= 1; 		stt_tx<=12; fb <=1; end		//  bit stop và fb lai cho module top_level la da truyen xong du lieu		
			endcase
