...
source "[list [file join [file dirname [info script]] sdc_params.tcl]]"
...
################################################################################
set pin_file_name "memphy_pins.tcl"
...
# instantiations.
################################################################################
proc add_requirements_for_instance {corename instance_name t_name board_name ISI_name amp_phy_use_flexible_timing} {
	upvar 1 $t_name t
	upvar 1 $board_name board
	upvar 1 $ISI_name ISI
	set instname "${instance_name}"
	...
	if {$ck_pll_clock_id == -1} {
		...
		post_message -type warning "memphy.sdc: Failed to find PLL clock for pins [join $pins(ck_p)]"
	} else {
		...
		if {$pll_ref_clk_id != -1} {
			...
			post_message -type info "memphy.sdc: Could not find PLL clocks for $ck_pll_clock. Creating PLL base clocks"
			# Attempt to recover
	...
	set resync_clock_pattern ${instname}|clk|*|altpll_component|auto_generated|pll1|clk\[3\]
	set resync_clock_id ""
	...
	if {$resync_pll_ref_clk_id != -1} {
		...
		post_message -type warning "memphy.sdc: Failed to find PLL input clock pin driving $resync_clock"
	}

	################################################################################
	# Find the TimeQuest name for the mimic clock. If it is not found, create one.
	set mimic_clock_pattern ${instname}|clk|*|altpll_component|auto_generated|pll1|clk\[4\]
	...
	set system_clock_pattern ${instname}|clk|*|altpll_component|auto_generated|pll1|clk\[1\]
	...
	#TODO: Do these parms need to be changed?
	...
	set capture_pattern ${instname}|dpio|dqs_group\[*\].dq\[*\].dqi|auto_generated|input_cell_*\[0\]|clk
	...
	set measure_pattern ${instname}|clk|DDR_CLK_OUT\[0\].ddr_clk_out_p|auto_generated|input_cell_h\[0\]|clk
	...
	# domain crossing.
	################################################################################
	set scan_clock_patterns [list ${instname}|clk|scan_clk|q 2]
	...
	if {$dqs_pll_clock_id == -1 || $dq_pll_clock_id == -1} {
		...
		post_message -type warning "memphy.sdc: Failed to find PLL clock for pins [join [get_all_dqs_pins $pins(dqsgroup)]]"
	} else {
	...
	foreach dqsgroup $pins(dqsgroup) {
		...
		if {$amp_phy_use_flexible_timing} {
			# DQS output clock
			...
			if {$amp_phy_use_flexible_timing && ($::quartus(nameofexecutable) ne "quartus_fit")} {
				lappend ::dqs_clocks $dqs_out_clockname
			}
		}
		# endif $amp_phy_use_flexible_timing
	...
	set_false_path -to [get_pins -compatibility_mode ${instname}|dpio|dqs_group\[*\].dq\[*\].dqi|auto_generated|input_*_*\[0\]|clrn]

	if {($::quartus(nameofexecutable) == "quartus_fit") || !$amp_phy_use_flexible_timing} {
	...
	set rd_wr_latency_ops [get_pins -compatibility_mode ${instname}|*seq_wrapper|*seq_inst|*dgrb|?d_lat*|clk]
	...
	set_false_path -from [get_pins -compatibility_mode  ${instname}|*seq_wrapper|*seq_inst|seq_mem_clk_disable*]
	...
	# the PLL reconfig block
	set clear_list [list \
		${instname}|clk|*pll|altpll_component|auto_generated|pll_lock_sync|clrn \
		${instname}|clk|global_pre_clear|clrn \
		${instname}|clk|reset_master_ams|clrn \
		${instname}|clk|mem_pipe|ams_pipe\[*\]|clrn \
		${instname}|clk|mem_clk_pipe|ams_pipe\[*\]|clrn \
		${instname}|clk|write_clk_pipe|ams_pipe\[*\]|clrn \
		${instname}|clk|measure_clk_pipe|ams_pipe\[*\]|clrn \
		${instname}|clk|resync_clk_pipe|ams_pipe\[*\]|clrn \
		${instname}|clk|clk_div_reset_ams_n_r|clrn \
		${instname}|clk|clk_div_reset_ams_n|clrn \
		${instname}|clk|pll_reconfig_reset_ams_n_r|clrn \
		${instname}|clk|pll_reconfig_reset_ams_n|clrn \
	]
	...
	set_false_path -from [get_pins -compatibility_mode *phy|*seq_wrapper|*seq_inst|*|seq_mmc_start*|*] -to [get_keepers *phy|*mmc|seq_mmc_start_metastable*]
	set_false_path -from [get_pins -compatibility_mode *phy|*mmc|mimic_done_out*] -to [get_keepers *phy|*seq_wrapper|*seq_inst|*dgrb|*v_mmc_seq_done_1r*]
	...
	if {$ac_pll_clock_id == -1} {
		...
		post_message -type warning "memphy.sdc: Failed to find PLL clock for pins [join $pins(addrcmd)]"
	} else {
	...
	#TODO: Ignore TG and bridge?
	set_false_path -from *tg*
	set_false_path -to *tg*
	set_false_path -from *status_bridge*
	set_false_path -to *status_bridge*

	if {$amp_phy_use_flexible_timing && (($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
...
foreach inst $instance_list {
	...
	add_requirements_for_instance $corename $inst t board ISI $amp_phy_use_flexible_timing
	#add_ddr_report_command "source [list [file join [file dirname [info script]] ${corename}_report_timing.tcl]]"
	add_ddr_report_command "source [list [file join [file dirname [info script]] memphy_report_timing.tcl]]"
}
