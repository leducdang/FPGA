...
// alt_u8 settime[4]= { RegisterAddr,0x20,0x46,0x07 };
...
// status = alt_avalon_i2c_master_tx(my_i2c,settime,sizeof(settime),ALT_AVALON_I2C_NO_INTERRUPTS);
 while(1)
 {
	 ...
	 IOWR_ALTERA_AVALON_PIO_DATA(LED1_BASE,led7_seg[ReadTempbuf[2]>>4 &0x0f]);  //hang chuc - hour
	 IOWR_ALTERA_AVALON_PIO_DATA(LED2_BASE,led7_seg[ReadTempbuf[2] &0x0f]); 	//hang don vi - hour

	 IOWR_ALTERA_AVALON_PIO_DATA(LED3_BASE,led7_seg[ReadTempbuf[1]>>4 &0x0f]);  //hang chuc - minute
	 IOWR_ALTERA_AVALON_PIO_DATA(LED4_BASE,led7_seg[ReadTempbuf[1] &0x0f]); 	//hang don vi - minute

	 IOWR_ALTERA_AVALON_PIO_DATA(LED5_BASE,led7_seg[ReadTempbuf[0]>>4 &0x0f]);  //hang chuc - second
	 IOWR_ALTERA_AVALON_PIO_DATA(LED6_BASE,led7_seg[ReadTempbuf[0] &0x0f]); 	//hang don vi - second
