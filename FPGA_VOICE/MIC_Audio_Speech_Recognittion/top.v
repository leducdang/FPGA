...
always @(posedge clk_16khz or negedge reset )
begin
	if(!reset)begin
            ...
            UART: 
				begin
					case(stt_uart)
						...
						5'd4:	begin
							if(uart_address_sram > 19'd47999) 
									begin
										stt_uart <= 0;
										start_uart_new <= 0;
										uart_address_sram <= 0;
				...
				begin
					case(stt_uart)		
						5'd1:	begin
							if(!busy_uart_new)
								begin
									data_in_uart <= {out_data_lut[send_lut], 32'd0};
									start_uart_new <= 1;
									stt_uart <= 2;
									data_byte <= 2;
								end
				...
				begin
					case(stt_uart)		
						5'd1:	begin
							if(!busy_uart_new)
								begin
									data_in_uart <= {data_in_FFT[uart_addr_ram], 32'd0};
									start_uart_new <= 1;
									stt_uart <= 2;
									data_byte <= 2;
								end
				...
				begin
					case(stt_uart)				
						...
						5'd1:	begin
							if(!busy_uart_new)
								begin
									data_in_uart <= {fft_real_ram[send_fft], 24'd0};
									start_uart_new <= 1;
									stt_uart <= 2;
									data_byte <= 3;
								end
						...
						5'd6: if(busy_uart_new)
							begin
								start_uart_new <= 0;
								stt_uart <= 7;
							end
						5'd7:	if(done_uart_new)
							begin
								stt_uart <= 8;	
							end	
					...
					/////////////////////////////////////////////////////
						...
						5'd8:	begin
							if(!busy_uart_new)
								begin
									data_in_uart <= {fft_imag_ram[send_fft], 24'd0};
									start_uart_new <= 1;
									stt_uart <= 9;
									data_byte <= 3;
								end
							end
						5'd9: if(busy_uart_new)
							begin
								start_uart_new <= 0;
								stt_uart <= 10;
							end
						5'd10:	if(done_uart_new)
							begin
								send_fft <= send_fft + 9'd1;
								stt_uart <= 11;	
							end
				...
				begin
					case(stt_uart)
						5'd1:	begin
							if(!busy_uart_new)
								begin
									data_in_uart <= {data_mag[send_fft], 16'd0};
									start_uart_new <= 1;
									stt_uart <= 2;
									data_byte <= 4;
								end
				...
				UART_MEL:
					begin
						...
						5'd1:	begin
							if(!busy_uart_new)
								begin
									data_in_uart <= mel_20_data[send_fft];
									start_uart_new <= 1;
									stt_uart <= 2;
									data_byte <= 6;
								end
